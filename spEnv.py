...
class SpEnv(gym.Env):
    ...
    def __init__(self, minLimit=None, maxLimit=None, operationCost = 0, observationWindow = 60, ensamble = None, callback = None, isOnlyShort=False, columnName = "iteration-1"):
        ...
        spTimeserie = pandas.read_csv('./datasets/'+MK+'Day.csv')[minLimit:maxLimit] # opening the dataset
        ...
        #considering 8 weeks
        self.weekData = MergedDataStructure(delta=2,filename="./datasets/"+MK+"Month.csv")# this DS allows me to obtain previous historical data with different resolution
        self.dayData = MergedDataStructure(delta=8,filename="./datasets/"+MK+"Week.csv")#  with low computational complexity
        ...
        for i in range(0,self.limit): 
            self.history.append({'Date' : Date[i], 'Open': Open[i], 'High': High[i], 'Low': Low[i], 'Close': Close[i], 'Volume': Volume[i]}) #self.history.append({'Date' : Date[i],'Time' : Time[i], 'Open': Open[i], 'High': High[i], 'Low': Low[i], 'Close': Close[i], 'Volume': Volume[i]})
    ...
    def getObservation(self, date):
        ...
        return  numpy.array(
            [list(
                map(
                    lambda x: (x["Volume"], (x["Close"]-x["Open"])/x["Open"]),
