...
class DeepQTrading:
    ...
    def __init__(self, model, explorations, trainSize, validationSize, testSize, outputFile, begin, end, nbActions, isOnlyShort, ensembleFolderName, operationCost=0):
        ...
        self.dates= pd.read_csv('./datasets/'+MK+'Day.csv')
        self.sp = pd.read_csv('./datasets/'+MK+'Day.csv')
        #Convert the pandas format to date and time format
        self.sp['Datetime'] = pd.to_datetime(self.sp['Date'], format="%Y-%m-%d") #self.sp['Datetime'] = pd.to_datetime(self.sp['Date'] + ' ' + self.sp['Time'])
        ...
        self.sp = self.sp.drop(['Date'], axis=1) #self.sp = self.sp.drop(['Time','Date'], axis=1)
    ...
    def run(self):
        ...
        begin = self.currentStartingPoint
        ...
        while(self.currentStartingPoint+self.walkSize <= self.endingPoint):
            ...
            while(trainMinLimit is None):
                try:
                    ...
                    print(self.currentStartingPoint)
                except:
                    self.currentStartingPoint+=datetime.timedelta(1,0,0,0,0,0,0)
                    print("EXCEPTION")  
            ...
            while(trainMaxLimit is None):
                try:
                    ...
                    print(self.currentStartingPoint+self.trainSize)
                except:
                    self.currentStartingPoint+=datetime.timedelta(1,0,0,0,0,0,0)
                    print("EXCEPTION")
            ...
            while(validMaxLimit is None):
                try:
                    ...
                    print(self.currentStartingPoint+self.trainSize+self.validationSize)
                except:
                    self.currentStartingPoint+=datetime.timedelta(1,0,0,0,0,0,0)
                    print("EXCEPTION")
            ...
            for eps in self.explorations:
                ...
                for i in range(0,eps[1]):
                    ...
                    trainEnv.resetEnv()
                    #Train the agent
                    self.agent.fit(trainEnv,nb_steps=floor(self.trainSize.days/4.8-self.trainSize.days/4.8*0.2),visualize=False,verbose=0) #days -> weeks
                    ...
                    self.agent.test(validEnv,nb_episodes=floor(self.validationSize.days/4.8-self.validationSize.days/4.8*0.2),visualize=False,verbose=0) #days -> weeks
                    ...
                    self.agent.test(testEnv,nb_episodes=floor(self.validationSize.days/4.8-self.validationSize.days/4.8*0.2),visualize=False,verbose=0) #days -> weeks
            ...
            ensambleValid.to_csv(self.ensembleFolderName+"/Output/ensemble/walk"+str(iteration)+"ensemble_valid.csv")
            ensambleTest.to_csv(self.ensembleFolderName+"/Output/ensemble/walk"+str(iteration)+"ensemble_test.csv")

        self.total=ValidationCallback()
        totalEnv = SpEnv(operationCost=self.operationCost,minLimit=self.sp.get_loc(begin),maxLimit=self.sp.get_loc(end),callback=self.total,isOnlyShort=self.isOnlyShort)
        self.agent.test(totalEnv,nb_episodes=floor((end-begin).days/4.8-(end-begin).days/4.8*0.2),visualize=False,verbose=0)
        (_,fnlCoverage,fnlAccuracy,fnlReward,fnlLongPerc,fnlShortPerc,fnlLongAcc,fnlShortAcc,fnlLongPrec,fnlShortPrec)=self.total.getInfo()
        print("FINAL MODEL:  fnlacc: " + str(fnlAccuracy)+ " fnlcov: " + str(fnlCoverage)+ " fnlrew: " + str(fnlReward))

        print("BENCHMARK MODEL:  bmkacc: " + str(fnlAccuracy)+ " bmkcov: " + str(fnlCoverage)+ " bmkrew: " + str(fnlReward))
